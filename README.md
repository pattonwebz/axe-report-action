...
The report itself is generated by [@pattonwebz/axe-a11y-report](https://github.com/pattonwebz/axe-a11y-report) — this action is a thin wrapper that calls that package and writes the result to the job summary. Want the same report somewhere other than a GitHub Actions job summary (locally, another CI system, a PR-comment bot)? Use that package directly instead.
...
`dist/` is committed on purpose: GitHub runs JavaScript actions straight from the checked-out repo without installing dependencies. `ncc` bundles `@pattonwebz/axe-a11y-report` straight into `dist/index.js`, so no runtime install step is needed either way.
